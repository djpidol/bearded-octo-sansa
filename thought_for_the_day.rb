...
def iod_request (api, params)
  ...
  res = Net::HTTP.get_response(uri)
  return job_results(JSON.parse(res.body)['jobID'])
end
...
##########
## Formatear output (un poco)
def sentiment_analysis(item)
  puts "Analysing #{item.title}"
  sa_text = speech_analysis(item.link)
  puts "Speech Analysis #{sa_text}"
  puts "Sentiment Analysis #{sentiment(sa_text.content)}"
end
